...
function ProductCard({ product, onAdd }: { product: Product; onAdd: (product: Product, variant: any) => void }) {
  ...
  const price = selectedVariant?.prices?.find(p => p.currency_code === "vnd");
...
export default function ShopPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  useEffect(() => {
    fetch(`${MEDUSA_URL}/store/products?limit=100`, {
      headers: { "x-publishable-api-key": process.env.NEXT_PUBLIC_MEDUSA_PUBLISHABLE_KEY || "" }
    })
      .then(r => r.ok ? r.json() : Promise.reject())
      .then(({ products: d }) => setProducts(d || []))
      .catch(() => setError(true))
      ...
      <section className="max-w-[1400px] mx-auto px-6 lg:px-12 pb-24">
        {error ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {[...Array(8)].map((_, i) => <div key={i} className="bg-white/5 rounded-2xl aspect-[4/5] animate-pulse" />)}
          </div>
        ) : filtered.length === 0 ? (
