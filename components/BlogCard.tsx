...
export default function BlogCard({ id, slug, title, excerpt, date, imageUrl, locale, readMoreBtnText, basePath = 'blog' }: BlogCardProps) {
  return (
    <div className="group border border-white/5 bg-charcoal/20 overflow-hidden rounded flex flex-col hover:border-gold/30 transition-colors">
      <Link href={`/${locale}/${basePath}/${slug}`} className="block relative aspect-[4/3] overflow-hidden">
      ...
      <div className="p-3 sm:p-4 md:p-8 flex flex-col flex-1">
        ...
        <Link href={`/${locale}/${basePath}/${slug}`} dangerouslySetInnerHTML={{ __html: title }} className="font-serif text-sm sm:text-base md:text-2xl font-light mb-2 md:mb-4 line-clamp-2 hover:text-gold transition-colors" />
        ...
        <Link href={`/${locale}/${basePath}/${slug}`} className="mt-auto border-b-[3px] border-double border-gold/40 text-center pb-2 pt-2 md:pt-4 text-gold text-[9px] sm:text-[10px] uppercase tracking-widest hover:border-gold hover:text-white transition-colors">
          {readMoreBtnText}
        </Link>
      </div>
    </div>
