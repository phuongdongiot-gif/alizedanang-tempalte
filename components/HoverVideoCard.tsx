  ...
  return (
    ...
    >
      <div className="relative aspect-[4/3] overflow-hidden">
        {/* Badges */}
        <div className="absolute top-4 left-4 z-20 bg-gold text-jet-black text-[10px] uppercase tracking-widest font-bold px-3 py-1 rounded shadow-md">
          {dict.badge}
        </div>
      ...
      <div className="p-8 flex flex-col flex-1 bg-jet-black/50 backdrop-blur-sm relative z-20">
         <div className="text-[10px] text-gold/80 tracking-widest uppercase mb-2 font-light">RESIDENCE</div>
         <h3 className="font-serif text-2xl font-light mb-2 text-white group-hover:text-gold transition-colors">{project.name}</h3>
         ...
         <div className="mt-auto flex justify-between items-center border-t border-white/5 pt-6">
           <span className="text-lg font-light text-white hover:text-white/80 transition-colors cursor-pointer">{dict.contactBtn}</span>
           <Link href={`/${locale}/projects/${project.slug || 'the-royal-hoi-an'}`} className="text-gold text-xs tracking-widest uppercase font-light group-hover:underline flex items-center gap-1">
             {dict.detailBtn}
