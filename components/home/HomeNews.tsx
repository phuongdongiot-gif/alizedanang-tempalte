  ...
  return (
    <section id="news" className="py-20">
      <div className="max-w-[1400px] mx-auto px-6 lg:px-12">
        ...
        <div className="grid grid-cols-2 md:grid-cols-2 gap-2 sm:gap-4 md:gap-12 pb-4 md:pb-0">
          {wpPosts && wpPosts.length > 0 ? (
            wpPosts.map((post: any, idx: number) => {
              ...
              return (
                <Link href={`/${locale}/blog/${post.slug}`} key={idx} className="flex flex-col sm:flex-row gap-2 md:gap-8 group cursor-pointer border border-transparent hover:bg-white/5 p-2 rounded transition-colors w-full">
                  ...
                  <div className="sm:w-1/2 flex flex-col justify-center mt-1 sm:mt-0">
                    <span className="text-gold text-[8px] md:text-[10px] font-bold tracking-widest mb-1 md:mb-4">{date}</span>
                    <h3 className="font-serif text-[11px] sm:text-sm md:text-xl font-light mb-1 md:mb-4 line-clamp-2 group-hover:text-gold transition-colors">{title}</h3>
                    <p className="text-[9px] md:text-sm font-light text-champagne/60 line-clamp-2 md:line-clamp-3 leading-relaxed">{desc}</p>
                  </div>
                </Link>
              );
            })
