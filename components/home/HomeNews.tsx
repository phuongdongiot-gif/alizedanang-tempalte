        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 md:gap-8 pb-4 md:pb-0">
          {wpPosts && wpPosts.length > 0 ? (
            wpPosts.map((post: any, idx: number) => {
              ...
              return (
                <Link href={`/${locale}/blog/${post.slug}`} key={idx} className="flex flex-col gap-2 md:gap-3 group cursor-pointer border border-transparent hover:border-white/5 hover:bg-charcoal/20 p-2 md:p-4 transition-all shadow-sm w-full">
                  ...
                  <div className="w-full flex flex-col justify-start mt-2">
                    ...
                    <h3 className="font-serif text-[13px] sm:text-base md:text-xl font-light mb-1 md:mb-2 line-clamp-2 group-hover:text-gold transition-colors">{title}</h3>
